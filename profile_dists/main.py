...
from profile_dists.utils import (
    process_profile,
    is_file_ok,
    filter_columns,
    count_missing_data,
...
def parse_args():
    ...
    parser = ArgumentParser(
        description="Profile Dists: Calculate genetic distances based on allele profiles v. {}".format(
            __version__
        ),
        formatter_class=CustomFormatter,
    )
    parser.add_argument(
        "--query", "-q", type=str, required=True, help="Query allelic profiles"
    )
    parser.add_argument(
        "--ref", "-r", type=str, required=True, help="Reference allelic profiles"
    )
    parser.add_argument(
        "--outdir", "-o", type=str, required=True, help="Result output files"
    )
    parser.add_argument(
        "--outfmt",
        "-u",
        type=str,
        required=False,
        help="Out format [matrix, pairwise]",
        default="matrix",
    )
    parser.add_argument(
        "--file_type",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Distance method raw hamming or scaled difference [hamming, scaled]",
        default="scaled",
    )
    parser.add_argument(
        "--missing_thresh",
        ...
        type=float,
        required=False,
        help="Maximum percentage of missing data allowed per locus (0 - 1)",
        default=1.0,
    )
    parser.add_argument(
        "--sample_qual_thresh",
        ...
        type=float,
        required=False,
        help="Maximum percentage of missing data allowed per sample (0 - 1)",
        default=1.0,
    )
    parser.add_argument(
        "--match_threshold",
    ...
    parser.add_argument(
        "-s", "--skip", required=False, help="Skip QA/QC steps", action="store_true"
    )
    parser.add_argument(
        "--columns",
        required=False,
        help="Single column file with one column name per line or list of columns comma separate",
        default=None,
    )
    parser.add_argument(
        "-n",
        "--count_missing",
        required=False,
        help="Count missing as differences",
        action="store_true",
    )
    parser.add_argument(
        "-p",
...
def run_profile_dists(params):
    ...
    if not force and os.path.isdir(outdir):
        print(
            f"folder {outdir} already exists, please choose new directory or use --force"
        )
        sys.exit()

    if os.path.isdir(outdir):
        print(
            f"folder {outdir} already exists, and force specified, cleaning up directory"
        )
        files = [
            os.path.join(outdir, "allele_map.json"),
            os.path.join(outdir, f"query_profile.{file_type}"),
            os.path.join(outdir, f"ref_profile.{file_type}"),
            os.path.join(outdir, "dists.parquet"),
    ...
    if file_type not in ["text", "parquet"]:
        print(f"Supplied filetype does not match [text, parquet]: {outfmt} ")
        sys.exit()

    if dist_method not in ["hamming", "scaled"]:
    ...
    sys.stdout.flush()
    with open(os.path.join(outdir, "allele_map.json"), "w", encoding="utf-8") as fh:
    ...
    if columns is None:
        if len(common_cols) == 0:
            print(
                f"Error there are no columns in common between: {query_profile}\t{ref_profile}"
            )
            sys.exit()
        ...
        if len(set(columns) & set(qdf.columns.values.tolist())) == 0:
            print(
                f"Error there are no columns in common between: {query_profile}\t{columns}"
            )
            sys.exit()
        if len(set(columns) & set(rdf.columns.values.tolist())) == 0:
            print(
                f"Error there are no columns in common between: {ref_profile}\t{columns}"
            )
            sys.exit()
    ...
    cols_to_remove = []
    if not skip:
        print("Performing QA/QC on input allele profiles")
        ...
        for col in qmissing:
            count = qmissing[col]
            if col not in rmissing:
                rmissing[col] = 0
            rmissing[col] += count
            if rmissing[col] > missing_threshold:
                cols_to_remove.append(col)

        run_data["loci_removed"] = sorted(
            list(set(run_data["loci_removed"]) | set(cols_to_remove))
        )
    ...
    run_data["query_profile_info"]["num_samples_pass"] = run_data["query_profile_info"][
        "num_samples"
    ]
    run_data["ref_profile_info"]["num_samples"] = len(qlabels)
    run_data["ref_profile_info"]["num_samples_pass"] = run_data["ref_profile_info"][
        "num_samples"
    ]

    if not skip:
        print("Performing QA/QC on input sample profiles")
        # Remove poor quality samples from the comparisons
        query_missing_data_counts = get_missing_loci_counts(
            qprofiles, qlabels, len(common_cols)
        )
        ref_missing_data_counts = get_missing_loci_counts(
            rprofiles, rlabels, len(common_cols)
        )
        query_samples_to_remove = flag_samples(
            query_missing_data_counts, sample_qual_thresh
        )
        run_data["query_profile_info"]["failed_samples"] = query_samples_to_remove
        run_data["query_profile_info"]["num_samples_pass"] = run_data[
            "query_profile_info"
        ]["num_samples"] - len(query_samples_to_remove)
        ref_samples_to_remove = flag_samples(
            ref_missing_data_counts, sample_qual_thresh
        )
        run_data["ref_profile_info"]["failed_samples"] = ref_samples_to_remove
        run_data["ref_profile_info"]["num_samples_pass"] = run_data["ref_profile_info"][
            "num_samples"
        ] - len(ref_samples_to_remove)

        qlabels, qprofiles = filter_samples(
            qlabels, qprofiles, set(query_samples_to_remove)
        )
        rlabels, rprofiles = filter_samples(
            rlabels, rprofiles, set(ref_samples_to_remove)
        )

        print(f"Prefilter Query size {len(qdf)}")
        qdf = qdf.drop(list(query_samples_to_remove))
        print(f"Postfilter Query size {len(qdf)}")
        rdf = rdf.drop(list(ref_samples_to_remove))
    ...
    run_data["query_profile_info"]["parsed_file_path"] = os.path.join(
        outdir, f"query_profile.{file_type}"
    )
    write_profiles(rdf, os.path.join(outdir, f"ref_profile.{file_type}"), file_type)
    run_data["ref_profile_info"]["parsed_file_path"] = os.path.join(
        outdir, f"ref_profile.{file_type}"
    )
    sys.stdout.flush()
    ...
    if len(qprofiles) == 0 or len(rprofiles) == 0:
        print(
            f"Error filtering parameters are too stringent there are 0 samples remaining: query:{len(qprofiles)} ref:{len(rprofiles)}"
        )
        sys.exit()
    ...
    with pq.ParquetWriter(
        dist_matrix_file, schema=pq.ParquetFile(dist_files[0]).schema_arrow
    ) as writer:
